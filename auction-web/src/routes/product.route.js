...
router.get("/category", async (req, res) => {
  try {
    ...
    res.render("vwProduct/list", result);
  } catch (error) {
    console.error("Error in category route:", error);
    return res
      .status(500)
      .render("500", { message: "An error occurred while loading products" });
  }
});
...
router.get("/search", async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .render("500", { message: "An error occurred while searching products" });
  }
});
