...
router.get("/signin", function (req, res) {
  const success_message = getAndClearSuccessMessage(req);
...
router.post("/signin", async function (req, res) {
  ...
  const { user, error, unverified } = await AuthService.loginWithCredentials(
    email,
    password,
  );
  ...
  establishAuthSession(req, user);
  const returnUrl = getAndClearReturnUrl(req);
...
router.post("/verify-email", async (req, res) => {
  ...
  setSuccessMessage(req, "Your email has been verified. You can sign in now.");
  return res.redirect("/account/signin");
});
...
/**
 * Handles OAuth callback by establishing session and redirecting.
 * Eliminates code duplication across OAuth providers.
 */
function handleOAuthCallback(req, res) {
  establishAuthSession(req, req.user);
  res.redirect("/");
}
