...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Get formatted messages from service
      const viewData = await orderService.getFormattedMessages(orderId, userId);

      // Render Handlebars partial to HTML string
      res.render(
        "partials/chatMessages",
        {
          messages: viewData,
