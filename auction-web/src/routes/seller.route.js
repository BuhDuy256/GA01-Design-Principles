...
router.get("/products/sold", async function (req, res) {
  ...
  const productsWithReview = await reviewService.enrichProductsWithReviews(
    products,
    sellerId,
  );
...
router.post("/products/:id/cancel", async function (req, res) {
  try {
    ...
    await sellerService.cancelProductWithRating({
      productId,
      sellerId,
      reason,
...
router.post("/products/:id/append-description", async function (req, res) {
  try {
    ...
    const product = await productService.appendDescription(
      productId,
      sellerId,
      description.trim(),
    );
    ...
    notificationService.notifyUsersAboutDescriptionUpdate(
      product,
      description.trim(),
      sellerId,
      productUrl,
...
router.get("/products/:id/description-updates", async function (req, res) {
  try {
    ...
    const updates = await sellerService.getDescriptionUpdates(
      productId,
      sellerId,
    );
...
router.put(
  ...
  async function (req, res) {
    try {
      ...
      await sellerService.updateDescriptionContent(updateId, sellerId, content);
...
router.delete(
  ...
  async function (req, res) {
    try {
      ...
      await sellerService.deleteDescriptionUpdate(updateId, sellerId);
